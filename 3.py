...
while i < len(x):
    ...
    while j < len(x[i]):
        if x[i][j].isdigit():
            ...
            if j == len(x[i]) - 1 or not x[i][j+1].isdigit():
                numbers_map[(i, start)] = j+1-start
                start = -1
...
def in_range(i, j):
    return i >= 0 and i < len(x) and j >= 0 and j < len(x[i])

# return start pos of number
# throws exception if input pos does not contain a digit
def find_number_start(pos):
    if not x[pos[0]][pos[1]].isdigit():
        raise Exception("Pass in digit")
    
    start = pos
    while in_range(start[0], start[1] - 1) and x[start[0]][start[1] - 1].isdigit():
...
def gear_val(pos):
    ...
    ret_val = 1            
    if len(adj_nums) == 2:
        for pos_num in adj_nums:
            ret_val *=  int(x[pos_num[0]][pos_num[1]:pos_num[1] + numbers_map[pos_num]])
    else:
        ret_val = 0
...
for pos, length in numbers_map.items():
